  ...
  return (
    ...
    <LinearGradient 
      ...
      <Text
        ...
        {" "} Prime'a Dahil</Text>
      <View>
        <Button theme="primary" size={35} text={"Watch Now"} icon={"play"} onPress={() => handlePress()}></Button>
        <Button theme="secondary" size={32} text={"İndir"} text={isDownloaded ? "I Love Kebab" : "Download"} IconColor={"#fff"} icon={isDownloaded ? "check" : "download"} onPress={handleAdd}></Button></View>
      <View style={{ flexDirection: "row" }}>
        <Button theme="tertiary" size={40} IconColor={"#fff"} icon={"play-circle-outline"} />
        <Button theme="tertiary" size={40} IconColor={"#fff"} onPress={()=>handleAddWatchList()} icon={isAdded ? "check" : "plus"} />
      ...
      <View style={{ marginHorizontal: 10 }}>
        <Text style={{ ...styles.prime_text, color: "#808080" }}>Year {movie.release_date.slice(0, 4)}</Text>
      </View>
      <View style={{ marginHorizontal: 10 }}>
        <Text style={{ ...styles.prime_text, color: "#808080" }}>Original language {movie.original_language.toUpperCase()}</Text>
      </View>
    </ScrollView>
  );
};
